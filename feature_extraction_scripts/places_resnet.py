...
IMAGE_DIRS = [
    "data/raw/images/gsv",
    "data/raw/images/mapillary"]
MATCHED_CSV = "data/processed/clusters_matched_filtered.csv"
OUTPUT_DIR = "resnet_places_features"
...
def load_places365_resnet():
    model = models.resnet50(weights=None)
    
    # Places365 was trained with a 365-class head
    ...
    # The checkpoint uses "state_dict" key with "module." prefix
    state_dict = {
        k.replace("module.", ""): v
    ...
    feature_extractor = nn.Sequential(
        *list(model.children())[:-1],  # includes avgpool
    )
    feature_extractor.eval().to(device)
    return feature_extractor

model = load_places365_resnet()
...
print(f"Output shape: {out.shape}")  # should be torch.Size([1, 2048, 1, 1]) or [1, 2048]
...
def extract_features(image_path):
    try:
        ...
        return features.squeeze().cpu().numpy()  # flattens (1,2048,1,1) → (2048,)
